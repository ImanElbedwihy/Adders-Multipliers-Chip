...
module adder ( A, B, Cin, Sum, Cout, overflow );
  input [31:0] A;
  input [31:0] B;
  output [31:0] Sum;
  input Cin;
  ...
  XOR2X1 U3 ( .IN1(B[31]), .IN2(A[31]), .Q(n3) );
  XNOR2X1 U4 ( .IN1(A[31]), .IN2(Sum[31]), .Q(n2) );
  adder_DW01_add_0 add_1_root_add_8_2 ( .A({1'b0, A}), .B({1'b0, B}), .CI(Cin), 
        .SUM({Cout, Sum}) );
  NOR2X0 U6 ( .IN1(n2), .IN2(n3), .QN(overflow) );
endmodule
